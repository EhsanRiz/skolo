...
export default function Grades() {
  ...
  if (loading) return <div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>Loading grades...</div>

  return (
    <div style={{ maxWidth: 600, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700, color: '#0f172a', margin: 0 }}>Grades</h1>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {hasAnyData ? (
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: 11, color: '#64748b' }}>results available</div>
                  </div>
                ) : (
                  ...
                  {TERMS.map(t => (
                    <button key={t} onClick={() => setActiveTerm(prev => ({ ...prev, [child.id]: t }))}
                      style={{
                        ...
                        background: term === t ? '#1d4ed8' : '#f1f5f9',
                        color: term === t ? '#fff' : child.terms?.[t] ? '#0f172a' : '#cbd5e1'
                      }}>
                      T{t}
                    </button>
                  ))}
                ...
                {termData ? (
                  <>
                      ...
                      <tbody>
                        {termData.results.map(r => {
                          ...
                          return (
                            <tr key={r.id} style={{ borderTop: '1px solid #f1f5f9' }}>
                              <td style={{ padding: '8px 0', color: '#0f172a' }}>{r.subject}</td>
                        ...
                        {termData.average != null && (
                          <tr style={{ borderTop: '2px solid #e2e8f0' }}>
                            <td style={{ padding: '8px 0', fontWeight: 700, color: '#0f172a' }}>Average</td>
                            <td style={{ padding: '8px 0', textAlign: 'center', fontWeight: 700, color: '#1d4ed8' }}>{termData.average}%</td>
                            <td style={{ padding: '8px 0', textAlign: 'center' }}>
                    ...
                    <button onClick={() => downloadReport(child.id, term)}
                      ...
                      style={{
                        width: '100%', padding: '10px', borderRadius: 8, border: '1.5px solid #1d4ed8',
