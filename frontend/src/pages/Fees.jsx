...
function exportArrears(ledger, sym, schoolName) {
  ...
  ws['!cols'] = [22,12,10,28,12,12,12,12,10].map(w => ({ wch: w }))
...
export default function Fees() {
  ...
  const loadSummary = async () => {
    try { const { data } = await api.get(`/fee-ledger/summary?year=${year}`); setSummary(data) }
    catch { }
  }
  ...
  const saveSched = async e => {
    ...
    try { await api.post('/fees/schedules', sf); setShowSched(false); toast.success('Fee schedule created'); loadOld() }
    catch (err) { toast.error(err.response?.data?.error || 'Failed') }
    ...
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          {tab === 'ledger' && <>
            <button style={t.btn.ghost} onClick={() => setShowSched(true)}>+ Manual schedule</button>
            <button style={t.btn.primary} onClick={() => setShowGenerate(true)}>⚡ Generate fees</button>
          </>}
          {tab === 'payments' && <button style={t.btn.primary} onClick={() => setShowPay(true)}>+ Record payment</button>}
        </div>
      </div>

      {/* Summary cards — ledger tab only */}
      {tab === 'ledger' && summary && (
          ...
          {TABS.map(tb => (
            <button key={tb} onClick={() => setTab(tb)} style={{
              ...
              {tb === 'ledger' && overdueCount > 0 && (
                <span style={{ marginLeft:8, background:'#dc2626', color:'#fff', borderRadius:20, padding:'1px 7px', fontSize:11 }}>{overdueCount}</span>
              )}
            </button>
          ))}
        </div>
          ...
          <div style={{ display:'flex', gap:10, alignItems:'center' }}>
            <select value={ledgerFilter.month} onChange={e => setLedgerFilter(f=>({...f,month:e.target.value}))}
              ...
              {Array.from({length:12},(_,i)=>{
                const y = year; const m = String(i+1).padStart(2,'0')
                return <option key={i} value={`${y}-${m}`}>{MONTHS[i]} {y}</option>
              })}
            </select>
            ...
            <button onClick={() => { exportArrears(ledger, sym, schoolName); toast.success('Exported to Excel') }}
      ...
      {tab === 'ledger' && (
        <div style={{ background:'#fff', borderRadius:14, boxShadow:'0 1px 3px rgba(0,0,0,.06)', overflow:'hidden' }}>
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead><tr>
              {['Learner','Grade','Description','Due date','Due','Paid','Balance','Status',''].map(h=>
                <th key={h} style={t.th}>{h}</th>
              )}
            </tr></thead>
            <tbody>
              {loadingLedger && (
                <tr><td colSpan={9} style={{ ...t.td, textAlign:'center', color:'#94a3b8', padding:40 }}>Loading…</td></tr>
              )}
              {!loadingLedger && ledger.map(row => {
                ...
                return (
                  <tr key={row.id} style={{ background: isOverdue ? '#fff9f9' : '' }}>
                    ...
                    <td style={t.td}>
                      {row.learners?.classes?.grades?.name || '—'}{' '}
                      {row.learners?.classes?.name || ''}
                    </td>
                    ...
                    <td style={t.td}>
                      {row.status !== 'paid' && (
                        <button onClick={() => openPay(row)} style={{ ...t.btn.primary, padding:'5px 12px', fontSize:12 }}>
                          Pay
                        </button>
                      )}
                    </td>
              ...
              {!loadingLedger && ledger.length === 0 && (
                <tr><td colSpan={9} style={{ ...t.td, textAlign:'center', color:'#94a3b8', padding:48 }}>
                  <div style={{ marginBottom:8, fontSize:18 }}>📋</div>
                  No fee entries yet. Click <strong>Generate fees</strong> to create entries for the selected month.
                </td></tr>
              )}
            </tbody>
          </table>
        ...
        <div style={{ background:'#fff', borderRadius:14, boxShadow:'0 1px 3px rgba(0,0,0,.06)', overflow:'hidden' }}>
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            ...
            <tbody>
              ...
              {!payments.length && <tr><td colSpan={6} style={{ ...t.td, color:'#94a3b8', textAlign:'center', padding:40 }}>No manual payments recorded.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
        ...
        <div style={t.overlay} onClick={e => e.target===e.currentTarget && setShowGenerate(false)}>
          <div style={{ ...t.modal, maxWidth:440 }}>
            ...
            <p style={{ fontSize:13, color:'#64748b', marginBottom:20 }}>
              Creates one fee entry per learner based on active fee plans. Existing entries are skipped.
            </p>
            <form onSubmit={generate}>
              ...
              <label style={t.label}>Override due date <span style={{ color:'#94a3b8', fontWeight:400 }}>(optional — uses fee plan default if blank)</span></label>
              ...
              <div style={{ background:'#fffbeb', border:'1px solid #fcd34d', borderRadius:9, padding:'10px 14px', marginBottom:14, fontSize:13, color:'#78350f' }}>
                ⚠ Make sure you have created Fee Plans in Settings → Fee Plans first.
              </div>
      ...
      {payEntry && (
        <div style={t.overlay} onClick={e => e.target===e.currentTarget && setPayEntry(null)}>
          <div style={{ ...t.modal, maxWidth:400 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14, padding:'12px 14px', background:'#f8fafc', borderRadius:10 }}>
              <div>
            ...
            <form onSubmit={recordPay}>
              <label style={t.label}>Amount to pay ({sym}) *</label>
              ...
              <input style={t.input} value={payForm.notes} onChange={e => setPayForm(f=>({...f,notes:e.target.value}))} placeholder="e.g. Cash received at office" />
        ...
        <div style={t.overlay} onClick={e => e.target===e.currentTarget && setShowSched(false)}>
          <div style={t.modal}>
            ...
            <form onSubmit={saveSched}>
              ...
              <input style={t.input} value={sf.name} onChange={e => setSf(f=>({...f,name:e.target.value}))} required />
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
              ...
              <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
                ...
                <button type="submit" style={t.btn.primary} disabled={saving}>{saving?'Saving…':'Save'}</button>
              </div>
            </form>
          </div>
        </div>
