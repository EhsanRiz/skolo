...
export default function Waivers() {
  ...
  return (
    <div>
      ...
      {/* Filter tabs */}
      <div style={{ display:'flex', gap:2, background:'#f7f7f7', borderRadius:10, padding:4, marginBottom:20, width:'fit-content' }}>
        {[
          ...
          <button key={f.key} onClick={() => setFilter(f.key)}
            style={{ padding:'7px 16px', borderRadius:8, border:'none', cursor:'pointer', fontWeight:600, fontSize:13,
              ...
              color: filter===f.key ? '#003049' : '#6b7280',
            ...
            <thead>
              <tr style={{ background:'#fafafa' }}>
                {['Learner','Grade','Amount','Reason','Requested by','Date','Status',''].map(h =>
                  <th key={h} style={{ textAlign:'left', padding:'10px 16px', fontSize:11, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.6px' }}>{h}</th>
                )}
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr key={r.id} style={{ borderTop:'1px solid #f7f7f7' }}>
                    <td style={{ padding:'12px 16px', fontWeight:600, color:'#1f2937' }}>
                      ...
                      {r.learners?.reference_no && <span style={{ marginLeft:6, fontSize:11, color:'#9ca3af' }}>#{r.learners.reference_no}</span>}
                    </td>
                    ...
                    <td style={{ padding:'12px 16px', fontSize:13, color:'#6b7280' }}>{r.requested_user?.full_name || '—'}</td>
                    ...
                    <td style={{ padding:'12px 16px' }}>
                      ...
                      {r.status !== 'pending' && r.review_note && (
                        <button onClick={() => setSelected(r)}
                          style={{ padding:'5px 14px', background:'#fafafa', color:'#6b7280', border:'1px solid #e5e7eb', borderRadius:7, fontSize:12, fontWeight:600, cursor:'pointer' }}>
                          Details
                        </button>
                      )}
                    </td>
            ...
            <div style={{ background:'#faf5ff', border:'1px solid #e9d5ff', borderRadius:12, padding:'18px', marginBottom:20 }}>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))', gap:14 }}>
                {[
                  ...
                  <div key={l}>
                    <div style={{ fontSize:10, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.6px', marginBottom:3 }}>{l}</div>
              ...
              {selected.note && (
                <div style={{ marginTop:14, paddingTop:14, borderTop:'1px solid #e9d5ff' }}>
                  <div style={{ fontSize:10, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'0.6px', marginBottom:4 }}>Bursar note</div>
            ...
            {selected.status === 'pending' && isApprover && (
              <>
                <label style={{ display:'block', fontSize:13, fontWeight:600, color:'#374151', marginBottom:6 }}>
                  Note <span style={{ color:'#9ca3af', fontWeight:400 }}>(required for rejection, optional for approval)</span>
                </label>
                <textarea value={reviewNote} onChange={e => setReviewNote(e.target.value)}
                  placeholder="e.g. Approved — learner qualifies for hardship bursary"
                  style={{ width:'100%', padding:'10px 13px', border:'1.5px solid #e5e7eb', borderRadius:9, fontSize:14, outline:'none', marginBottom:16, resize:'vertical', minHeight:80, fontFamily:'inherit', boxSizing:'border-box' }} />
                <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
                  <button onClick={() => setSelected(null)}
                    style={{ padding:'9px 18px', background:'#f7f7f7', border:'none', borderRadius:9, fontWeight:600, cursor:'pointer', color:'#374151' }}>Cancel</button>
            ...
            {selected.status !== 'pending' && (
              <>
                ...
                <div style={{ display:'flex', justifyContent:'flex-end' }}>
                  <button onClick={() => setSelected(null)}
                    style={{ padding:'9px 18px', background:'#f7f7f7', border:'none', borderRadius:9, fontWeight:600, cursor:'pointer' }}>Close</button>
                </div>
              </>
            )}
          </div>
