...
.sa-stat:hover { box-shadow:0 6px 20px rgba(0,0,0,.1); transform:translateY(-2px); border-color:#e5e7eb; }
.sa-tab { padding:10px 20px; border:none; background:transparent; font-size:14px; font-weight:600; color:#9ca3af; cursor:pointer; border-bottom:2.5px solid transparent; transition:all .15s; white-space:nowrap; }
.sa-tab:hover { color:#4b5563; }
.sa-tab.active { color:#003049; border-bottom-color:#003049; }
.sa-table { width:100%; border-collapse:separate; border-spacing:0; font-size:13px; }
.sa-table th { text-align:left; padding:10px 14px; font-weight:700; font-size:11px; text-transform:uppercase; letter-spacing:.6px; color:#9ca3af; border-bottom:2px solid #f7f7f7; position:sticky; top:0; background:#fff; z-index:1; }
.sa-table td { padding:10px 14px; border-bottom:1px solid #fafafa; color:#374151; }
...
.sa-search { width:100%; padding:10px 14px; border:1.5px solid #e5e7eb; border-radius:10px; font-size:14px; outline:none; background:#fafafa; box-sizing:border-box; }
.sa-search:focus { border-color:#003049; background:#fff; }
.sa-drawer-overlay { position:fixed; inset:0; z-index:200; display:flex; justify-content:flex-end; }
.sa-drawer-bg { position:absolute; inset:0; background:rgba(0,48,73,0.5); backdrop-filter:blur(2px); }
...
const PIE_COLORS = ['#003049', '#16a34a', '#f7c548', '#ef4444', '#8b5cf6', '#ec4899']
const ROLE_COLORS = { admin: '#003049', principal: '#8b5cf6', bursar: '#f7c548', teacher: '#16a34a' }
const STATUS_COLORS = { trial: '#f7c548', active: '#16a34a', expired: '#ef4444', cancelled: '#9ca3af' }
    ...
    <div className="sa-card sa-stat" style={{ animationDelay: delay || '0ms' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div>
          ...
          <div style={{ fontSize:28, fontWeight:900, color: accent || '#1f2937', letterSpacing:'-0.5px', lineHeight:1 }}>{value}</div>
...
function TimeAgo({ date }) {
  if (!date) return <span style={{ color:'#d1d5db' }}>Never</span>
...
function EmptyState({ icon, title, sub }) {
  return (
    <div style={{ textAlign:'center', padding:'60px 20px', color:'#9ca3af' }}>
      ...
      <div style={{ fontSize:16, fontWeight:700, color:'#6b7280' }}>{title}</div>
  ...
  if (loading) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#fafafa' }}>
      <div style={{ textAlign:'center' }}>
        <div style={{ fontSize:32, fontWeight:900, color:'#1f2937', letterSpacing:'-0.5px' }}>4D Platform Admin</div>
        <div style={{ fontSize:14, color:'#9ca3af', marginTop:8 }}>Loading dashboard…</div>
      </div>
    </div>
  )
  ...
  if (error) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#fafafa' }}>
      <div style={{ textAlign:'center', color:'#ef4444' }}>
        ...
        <button onClick={loadOverview} style={{ marginTop:16, padding:'8px 20px', background:'#003049', color:'#fff', border:'none', borderRadius:8, cursor:'pointer', fontWeight:600 }}>Retry</button>
      </div>
    </div>
  )
  ...
  return (
    <>
      ...
      <div style={{ minHeight:'100vh', background:'#fafafa' }}>
        {/* ── Top bar ── */}
        <div style={{ background:'#1f2937', color:'#fff', padding:'16px 32px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            <div style={{ width:36, height:36, borderRadius:10, background:'linear-gradient(135deg, #003049, #7c3aed)', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:900, fontSize:14 }}>4D</div>
            <div>
              <div style={{ fontWeight:800, fontSize:16, letterSpacing:'-0.3px' }}>Skolo Platform Admin</div>
              <div style={{ fontSize:11, color:'#9ca3af' }}>Manage all schools & users</div>
            </div>
          </div>
          <button onClick={() => { localStorage.removeItem('sa_token'); navigate('/super-admin/login') }}
            style={{ padding:'7px 16px', background:'rgba(255,255,255,0.08)', color:'#9ca3af', border:'1px solid rgba(255,255,255,0.1)', borderRadius:8, cursor:'pointer', fontSize:13, fontWeight:600 }}>
            Sign out
          </button>
        </div>
        ...
        <div style={{ maxWidth:1300, margin:'0 auto', padding:'24px 32px 60px' }}>
          ...
          {tab === 'overview' && o && (
            <>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))', gap:16, marginBottom:28 }}>
                <StatCard label="Total Schools" value={o.totalSchools} sub={`${o.activeSchools} active (30d)`} accent="#003049" icon="🏫" delay="0ms" />
                ...
                <StatCard label="Logins This Week" value={o.loginsThisWeek} sub={`${o.loginsThisMonth} this month`} accent="#f7c548" icon="🔐" delay="180ms" />
              </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))', gap:16, marginBottom:28 }}>
                ...
                <div className="sa-card">
                  <div style={{ fontSize:15, fontWeight:800, color:'#1f2937', marginBottom:16 }}>Users by Role</div>
                  {rolePie.length > 0 ? (
                ...
                {/* Schools by Status */}
                <div className="sa-card">
                  <div style={{ fontSize:15, fontWeight:800, color:'#1f2937', marginBottom:16 }}>Schools by Status</div>
                  {statusPie.length > 0 ? (
                    <ResponsiveContainer width="100%" height={220}>
                      <PieChart>
              ...
              {/* Recent schools */}
              <div className="sa-card">
                <div style={{ fontSize:15, fontWeight:800, color:'#1f2937', marginBottom:4 }}>Recently Registered Schools</div>
                <div style={{ fontSize:12, color:'#9ca3af', marginBottom:16 }}>Newest first</div>
                    ...
                    <tbody>
                      {(o.schools || []).slice(0, 10).map(s => (
                        <tr key={s.id} style={{ cursor:'pointer' }} onClick={() => { setTab('schools'); loadSchoolDetail(s.id) }}>
                          ...
                          <td>{s.country}</td>
                          <td><Badge text={s.subscription_status} color={STATUS_COLORS[s.subscription_status] || '#9ca3af'} /></td>
                          <td>{s.userCount}</td>
                          <td>{s.learnerCount}</td>
                          <td>{s.loginCount30d}</td>
                          <td><TimeAgo date={s.lastLogin} /></td>
                          <td style={{ color:'#9ca3af', fontSize:12 }}>{new Date(s.created_at).toLocaleDateString('en-ZA')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <tbody>
                      {filteredSchools.map(s => (
                        <tr key={s.id} style={{ cursor:'pointer' }} onClick={() => loadSchoolDetail(s.id)}>
                          <td style={{ fontWeight:700, color:'#1f2937' }}>{s.name}</td>
                          <td>{s.country}</td>
                          <td><Badge text={s.subscription_plan || 'free'} color="#003049" /></td>
                          <td><Badge text={s.subscription_status} color={STATUS_COLORS[s.subscription_status] || '#9ca3af'} /></td>
                          <td>{s.userCount}</td>
                          <td>{s.learnerCount}</td>
                          <td>{s.loginCount30d}</td>
                  ...
                  <div className="sa-drawer">
                    <div style={{ padding:'24px 28px 16px', borderBottom:'1px solid #f7f7f7', flexShrink:0 }}>
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                        <div>
                          <div style={{ fontSize:20, fontWeight:800, color:'#1f2937' }}>{schoolDetail.school.name}</div>
                          <div style={{ fontSize:13, color:'#9ca3af', marginTop:2 }}>{schoolDetail.school.country} · {schoolDetail.school.email || 'No email'}</div>
                        </div>
                        <button onClick={() => setSelectedSchool(null)} style={{ background:'#f7f7f7', border:'none', borderRadius:8, width:34, height:34, cursor:'pointer', fontSize:16, display:'flex', alignItems:'center', justifyContent:'center', color:'#6b7280' }}>✕</button>
                      </div>
                      <div style={{ display:'flex', gap:8, marginTop:12 }}>
                        <Badge text={schoolDetail.school.subscription_status} color={STATUS_COLORS[schoolDetail.school.subscription_status] || '#9ca3af'} />
                        <Badge text={schoolDetail.school.subscription_plan || 'free'} color="#003049" />
                      </div>
                    </div>
                    ...
                    <div style={{ flex:1, overflowY:'auto', padding:'20px 28px 28px' }}>
                      ...
                      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:12, marginBottom:24 }}>
                        {[
                          { label:'Users', value: schoolDetail.stats.users, accent:'#003049' },
                          { label:'Learners', value: schoolDetail.stats.activeLearners, accent:'#16a34a' },
                          { label:'Collection Rate', value: schoolDetail.stats.collectionRate + '%', accent: schoolDetail.stats.collectionRate >= 70 ? '#16a34a' : '#f7c548' },
                          { label:'Fee Due', value: schoolDetail.school.currency + schoolDetail.stats.totalDue.toLocaleString(), accent:'#1f2937' },
                          ...
                          { label:'Attendance Records', value: schoolDetail.stats.attendanceRecords.toLocaleString(), accent:'#6b7280' },
                          { label:'Exam Grades', value: schoolDetail.stats.examGrades.toLocaleString(), accent:'#6b7280' },
                          { label:'Created', value: new Date(schoolDetail.school.created_at).toLocaleDateString('en-ZA'), accent:'#6b7280' },
                        ].map((s, i) => (
                          <div key={i} style={{ background:'#fafafa', borderRadius:10, padding:'12px 14px' }}>
                            <div style={{ fontSize:10, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'.5px' }}>{s.label}</div>
                      ...
                      <div style={{ marginBottom:24 }}>
                        <div style={{ fontSize:14, fontWeight:800, color:'#1f2937', marginBottom:8 }}>Staff Accounts</div>
                        <table className="sa-table">
                          ...
                          <tbody>
                            {schoolDetail.users.map(u => (
                              <tr key={u.id}>
                                ...
                                <td><Badge text={u.role} color={ROLE_COLORS[u.role] || '#6b7280'} /></td>
                      ...
                      {/* Recent logins */}
                      <div style={{ marginBottom:24 }}>
                        <div style={{ fontSize:14, fontWeight:800, color:'#1f2937', marginBottom:8 }}>Recent Logins</div>
                        {schoolDetail.recentLogins.length > 0 ? (
                          <div style={{ maxHeight:200, overflowY:'auto' }}>
                            {schoolDetail.recentLogins.map(l => (
                              <div key={l.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 0', borderBottom:'1px solid #fafafa', fontSize:13 }}>
                                <div><span style={{ fontWeight:600 }}>{l.user_name}</span> <Badge text={l.user_role} color={ROLE_COLORS[l.user_role] || '#6b7280'} /></div>
                                <div style={{ color:'#9ca3af', fontSize:12 }}><TimeAgo date={l.logged_in_at} /></div>
                              </div>
                            ))}
                          </div>
                        ) : <div style={{ color:'#9ca3af', fontSize:13, padding:12 }}>No login records yet</div>}
                      </div>

                      {/* Recent activity */}
                      <div>
                        <div style={{ fontSize:14, fontWeight:800, color:'#1f2937', marginBottom:8 }}>Recent Activity</div>
                        {schoolDetail.recentActivity.length > 0 ? (
                          <div style={{ maxHeight:200, overflowY:'auto' }}>
                            {schoolDetail.recentActivity.map(a => (
                              <div key={a.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 0', borderBottom:'1px solid #fafafa', fontSize:13 }}>
                                <div><span style={{ fontWeight:600 }}>{a.user_name || 'System'}</span> · <span style={{ color:'#6b7280' }}>{a.action.replace(/_/g, ' ')}</span></div>
                                <div style={{ color:'#9ca3af', fontSize:12 }}><TimeAgo date={a.created_at} /></div>
                              </div>
                            ))}
                          </div>
                        ) : <div style={{ color:'#9ca3af', fontSize:13, padding:12 }}>No activity records yet</div>}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {tab === 'invites' && (
            <>
              ...
              <div className="sa-card" style={{ marginBottom:20 }}>
                <div style={{ fontSize:15, fontWeight:800, color:'#1f2937', marginBottom:4 }}>Invite a New School</div>
                <div style={{ fontSize:12, color:'#9ca3af', marginBottom:16 }}>Enter the email of the school admin. They'll receive a unique registration link valid for 7 days.</div>
                <div style={{ display:'flex', gap:10, alignItems:'flex-start' }}>
                  ...
                  <button
                    ...
                    style={{
                      padding:'10px 24px', background: inviteLoading ? '#9ca3af' : '#003049', color:'#fff',
                ...
                <div style={{ overflowX:'auto', maxHeight:'55vh', overflowY:'auto' }}>
                  <table className="sa-table">
                    ...
                    <tbody>
                      {invites.map(i => (
                        <tr key={i.id}>
                          <td style={{ fontWeight:600, color:'#1f2937' }}>{i.email}</td>
                          <td>
                            <Badge
                              text={i.status}
                              color={i.status === 'registered' ? '#16a34a' : i.status === 'expired' ? '#ef4444' : '#f7c548'}
                            />
                          </td>
                          <td>{i.school_name || '—'}</td>
                          <td style={{ color:'#6b7280', fontSize:12 }}>{new Date(i.created_at).toLocaleDateString('en-ZA')}</td>
                          <td style={{ color:'#9ca3af', fontSize:12 }}>{new Date(i.expires_at).toLocaleDateString('en-ZA')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <tbody>
                      {filteredLogins.map(l => (
                        <tr key={l.id}>
                          ...
                          <td style={{ color:'#6b7280', fontSize:12 }}>{l.user_email}</td>
                          <td><Badge text={l.user_role} color={ROLE_COLORS[l.user_role] || '#6b7280'} /></td>
                          <td>{l.school_name}</td>
                          <td style={{ fontFamily:'monospace', fontSize:12, color:'#9ca3af' }}>{l.ip_address || '—'}</td>
                          <td style={{ color:'#6b7280', fontSize:12 }}>{new Date(l.logged_in_at).toLocaleString('en-ZA')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  <tbody>
                    {activity.map(a => (
                      <tr key={a.id}>
                        ...
                        <td><Badge text={a.entity_type || '—'} color="#6b7280" /></td>
                        <td>{a.school_name}</td>
                        <td style={{ color:'#6b7280', fontSize:12 }}>{new Date(a.created_at).toLocaleString('en-ZA')}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {tab === 'usage' && usageStats && (
            <>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))', gap:16, marginBottom:28 }}>
                <StatCard label="Fee Entries" value={usageStats.totals.feeEntries.toLocaleString()} accent="#003049" icon="💰" delay="0ms" />
                ...
                <StatCard label="Active Learners" value={usageStats.totals.activeLearners.toLocaleString()} accent="#f7c548" icon="🎓" delay="180ms" />
              </div>

              {/* Usage by school bar chart */}
              <div className="sa-card">
                <div style={{ fontSize:15, fontWeight:800, color:'#1f2937', marginBottom:16 }}>Feature Usage by School</div>
                {(() => {
                  ...
                  return chartData.length > 0 ? (
                    <ResponsiveContainer width="100%" height={Math.max(300, chartData.length * 40)}>
                      <BarChart data={chartData} layout="vertical" margin={{ left:20, right:20, top:5, bottom:5 }}>
                        ...
                        <YAxis type="category" dataKey="name" tick={{ fontSize:11, fill:'#374151' }} width={150} />
                        <Tooltip />
                        <Bar dataKey="Fees" fill="#003049" radius={[0,4,4,0]} />
