...
router.post('/bulk', async (req, res) => {
  ...
  try {
    ...
    if (error) throw error
    logActivity({ school_id: req.user.school_id, user_id: req.user.id, action: 'grades_entered', entity_type: 'exam_grade', metadata: { subject, term, year, count: upserts.length } })
