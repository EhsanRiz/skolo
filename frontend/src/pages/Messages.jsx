    ...
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1f2937', margin: 0 }}>New message</h1>
        </div>

        {/* Parent search */}
        <div style={{ position: 'relative', marginBottom: 16 }}>
          <svg style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: '#9ca3af' }} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <input
        ...
        {loadingParents ? (
          <div style={{ textAlign: 'center', padding: 40, color: '#9ca3af' }}>Loading parents...</div>
          ...
          <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 1px 3px rgba(0,0,0,.06)', overflow: 'hidden' }}>
            {filteredParents.map((p, i) => (
              <div key={p.id} onClick={() => startConversation(p)} style={{
                ...
                borderBottom: i < filteredParents.length - 1 ? '1px solid #f7f7f7' : 'none',
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, fontSize: 14, color: '#1f2937' }}>{p.full_name}</div>
                ...
                <div style={{ color: '#d1d5db', fontSize: 18, flexShrink: 0 }}>&rsaquo;</div>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)', minHeight: 400 }}>
        ...
        <div style={{
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontWeight: 700, fontSize: 15, color: '#1f2937' }}>{getConvName(activeConv)}</div>
            <div style={{ fontSize: 12, color: '#6b7280', textTransform: 'capitalize', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {getConvRole(activeConv)}
              {getConvLearnerInfo(activeConv) && (
                <span style={{ color: '#003049', marginLeft: 6 }}>&middot; {getConvLearnerInfo(activeConv)}</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px', background: '#fafafa' }}>
          {messages.length === 0 && (
            <div style={{ textAlign: 'center', padding: 40, color: '#9ca3af', fontSize: 14 }}>
              ...
              <div key={msg.id} style={{
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    color: isMine ? 'rgba(255,255,255,0.45)' : '#9ca3af'
                  }}>
        ...
        <form onSubmit={sendMessage} style={{
          ...
          <input
            ...
            style={{
              flex: 1, padding: '10px 16px', borderRadius: 24, border: '1.5px solid #e5e7eb',
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1f2937', letterSpacing: '-0.3px' }}>Messages</h1>
          <p style={{ fontSize: 14, color: '#6b7280', marginTop: 2 }}>Communicate with parents</p>
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 14, boxShadow: '0 1px 3px rgba(0,0,0,.06)', overflow: 'hidden' }}>
          {filteredConvs.map((conv, i) => (
            <div key={conv.id} onClick={() => setActiveConv(conv)} style={{
              ...
              borderBottom: i < filteredConvs.length - 1 ? '1px solid #f7f7f7' : 'none',
              transition: 'background 0.1s'
            }}
            onMouseEnter={e => e.currentTarget.style.background = '#fafafa'}
            onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                  <div style={{ fontWeight: conv.unread_count > 0 ? 700 : 500, fontSize: 14, color: '#1f2937', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {getConvName(conv)}
                    <span style={{ fontSize: 11, color: '#9ca3af', fontWeight: 400, marginLeft: 8, textTransform: 'capitalize' }}>
                      {getConvRole(conv)}
                    </span>
                  </div>
                ...
                {getConvLearnerInfo(conv) && (
                  <div style={{ fontSize: 11, color: '#003049', marginTop: 1, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {getConvLearnerInfo(conv)}
                  </div>
                )}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 3 }}>
                  <div style={{
                    fontSize: 13, color: conv.unread_count > 0 ? '#374151' : '#9ca3af', whiteSpace: 'nowrap',
