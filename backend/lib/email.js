...
/**
 * Send a school registration invite email (from super admin)
 */
async function sendSchoolInviteEmail({ to, registerUrl, expiresIn }) {
  const html = `
<!DOCTYPE html>
<html>
<body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f1f5f9;color:#0f172a">
  <div style="max-width:520px;margin:32px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08)">
    ...
    <div style="padding:32px">
      <div style="font-size:13px;color:#64748b;margin-bottom:6px">You've been invited</div>
      <div style="font-size:20px;font-weight:800;color:#0f172a;margin-bottom:20px">Register your school on Skolo</div>
      <p style="font-size:14px;color:#374151;line-height:1.7;margin:0 0 20px">
        You've been invited to register your school on the Skolo platform — the all-in-one school management solution for fees, attendance, grades, and more.<br><br>
        Click the button below to get started. This link expires in <strong>${expiresIn}</strong>.
      </p>
      <a href="${registerUrl}" style="display:block;text-align:center;background:#0f2044;color:#fff;text-decoration:none;font-weight:700;font-size:15px;padding:16px 24px;border-radius:10px;margin-bottom:16px">
        Register my school →
      </a>
      <p style="font-size:12px;color:#94a3b8;text-align:center;margin:0">
        If you didn't expect this, you can ignore this email.
  ...
  try {
    await resend.emails.send({
      from: FROM, to,
      subject: 'You\'ve been invited to register on Skolo',
      html
    })
    return { sent: true }
  } catch (err) {
    console.error('School invite email failed:', err.message)
    return { sent: false, error: err.message }
  }
}

module.exports = { sendWaiverRequestEmail, sendWaiverDecisionEmail, sendInviteEmail, sendPasswordReset, sendSchoolInviteEmail }
