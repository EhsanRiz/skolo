...
.action-chip { display:inline-flex; align-items:center; gap:5px; padding:6px 14px; border-radius:8px; font-size:12px;
  font-weight:600; cursor:pointer; border:1.5px solid #e2e8f0; background:#fff; color:#374151;
  transition:all .12s; text-decoration:none; }
.action-chip:hover { background:#f8fafc; border-color:#cbd5e1; transform:translateY(-1px); }
.progress-bar-sm { height:5px; border-radius:10px; background:#f1f5f9; overflow:hidden; width:100%; }
.progress-fill-sm { height:100%; border-radius:10px; transition:width .6s ease; }
`

const GRADE_COLORS = { A:'#16a34a', B:'#2563eb', C:'#ca8a04', D:'#ea580c', F:'#dc2626' }
        ...
        {data.classes.map((tc, idx) => {
          ...
          return (
            <div key={tc.id} className="class-card" style={{ marginBottom: 16, animationDelay: `${idx * 80}ms` }}>
              ...
              <div onClick={() => toggle(tc.class_id)}
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {tc.subject && (
                      <span style={{ fontSize: 11, background: tc.is_home_class ? 'rgba(255,255,255,0.15)' : '#eff6ff', color: tc.is_home_class ? 'rgba(255,255,255,0.8)' : '#0f2044', padding: '2px 8px', borderRadius: 20, fontWeight: 600 }}>
                        {tc.subject}
                      </span>
                    )}
                  </div>
              ...
              <div style={{ display: 'flex', borderBottom: isOpen ? '1px solid #f1f5f9' : 'none', padding: '12px 20px', gap: 8 }}>
                {/* Attendance rate */}
                <div className="stat-pill">
                  <div style={{ fontSize: 18, fontWeight: 800, color: stats.attendanceRate != null ? (stats.attendanceRate >= 80 ? '#16a34a' : stats.attendanceRate >= 60 ? '#ca8a04' : '#dc2626') : '#cbd5e1' }}>
                    {stats.attendanceRate != null ? `${stats.attendanceRate}%` : '—'}
                  </div>
                  <div style={{ fontSize: 11, color: '#94a3b8', fontWeight: 500, marginTop: 2 }}>Attendance</div>
                ...
                <div className="stat-pill">
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                    <span style={{ fontSize: 18, fontWeight: 800, color: stats.avgMark != null ? '#0f172a' : '#cbd5e1' }}>
                      {stats.avgMark != null ? `${stats.avgMark}%` : '—'}
                    </span>
                    {stats.gradeLabel && (
                      <span style={{ fontSize: 12, fontWeight: 700, color: GRADE_COLORS[stats.gradeLabel] || '#94a3b8' }}>
                        {stats.gradeLabel}
                      </span>
                    )}
                  </div>
                  <div style={{ fontSize: 11, color: '#94a3b8', fontWeight: 500, marginTop: 2 }}>Avg mark</div>
                  {stats.avgMark != null && (
                    <div className="progress-bar-sm" style={{ marginTop: 6, maxWidth: 60 }}>
                      <div className="progress-fill-sm" style={{
                        width: `${stats.avgMark}%`,
                        background: GRADE_COLORS[stats.gradeLabel] || '#94a3b8'
                      }} />
                    </div>
                  )}
                </div>
                ...
                {/* Learner count */}
                <div className="stat-pill">
                  <div style={{ fontSize: 18, fontWeight: 800, color: '#0f172a' }}>{learners.length}</div>
                  <div style={{ fontSize: 11, color: '#94a3b8', fontWeight: 500, marginTop: 2 }}>Learners</div>
                </div>
              </div>

              {/* Quick actions */}
              <div style={{ padding: '10px 20px', display: 'flex', gap: 8, flexWrap: 'wrap', borderBottom: isOpen ? '1px solid #f1f5f9' : 'none' }}>
